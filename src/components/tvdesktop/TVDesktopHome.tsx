...
const InstallerApp = dynamic(() => import('@/components/InstallerApp/InstallerApp'), { ssr: false });
...
function Card({ recipe, shape, onMenu, later, onToggleLater, rank, inlaid, coll }: {
    ...
    return (
        ...
        >
            ...
            {/* Le visuel ouvre la recette. Jusqu'ici seul le TITRE le faisait :
                on cliquait la photo, il ne se passait rien, et il fallait viser
                une ligne de texte de quelques pixels sous la carte. Quand la
                vidéo tourne, elle recouvre le visuel et reçoit les clics — on
                la pilote donc toujours au lieu d'ouvrir la fiche par mégarde. */}
            <div
                className={styles.thumb}
                role="button"
                tabIndex={0}
                aria-label={`Ouvrir ${label(recipe)}`}
                onClick={() => openRecipe(recipe)}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openRecipe(recipe); } }}
            >
        ...
        <div className={`${styles.shell} ${sidebarOpen ? '' : styles.shellClosed}`}>
            ...
            <aside className={styles.sidebar}>
                ...
                <nav className={styles.navGroup}>
                    ...
                    {/* La visite guidée du bureau finit sur « installe-le » : sans
                        cette ligne, elle désignerait un bouton absent. Le composant
                        se retire tout seul quand l'app est déjà installée. */}
                    <InstallerApp
