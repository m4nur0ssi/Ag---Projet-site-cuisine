...
export default function RecipeDetails({ recipe, prevId, nextId, isModal = false }: RecipeDetailsProps) {
    ...
    return (
        <>
            ...
            {/* Fiche « Comme au resto ». Tout le contenu rédigé de ces fiches
                (plat préféré, ce qu'on a aimé, la table, l'adresse, les transports)
                dormait dans `steps` : le bloc d'onglets qui les affiche est coupé
                pour cette catégorie, et rien d'autre ne les rendait. */}
            {recipe.category === 'restaurant' && (
