                                    ...
                                    {r.locations && r.locations.length > 1 ? (
                                        <>
                                            ...
                                            {r.locations.map((loc, li) => (
                                                <a key={li} className={styles.restoCoordRow} href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((loc.name ? loc.name + ' ' : '') + loc.address)}`} target="_blank" rel="noopener noreferrer">
                                                    <span className={styles.restoCoordIcon}>•</span><span>{loc.name ? `${loc.name} — ${loc.address}` : loc.address}</span>
                                                </a>
                                            ))}
                                            <a className={styles.restoCoordRow} style={{ fontWeight: 700 }} href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(r.mapsQuery || recipe.title)}`} target="_blank" rel="noopener noreferrer">
